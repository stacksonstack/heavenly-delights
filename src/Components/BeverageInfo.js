...
function BeverageInfo({name, desc, ml, onSale, price, salePrice, isAvailable, image, closeUp }) {
  ...
  return (
      ...
      <div className="info-price">{onSale? <> <p id="info-price"style={{textDecoration:'line-through'}}>${price}</p> <p id="info-price">${salePrice}</p> </>: <p id="info-price">${price}</p>}</div>
      <div className="info-avail">{isAvailable ? null : <p id="info-avail" style={{ color: "rgb(234, 26, 6)" }}>Sold Out</p> }
      </div>
